...
export default function RegisterUser() {
  ...
  const validationSchema = Yup.object({
    ...
    phone: Yup.string()
      ...
      .matches(/^\d{10}$/, 'رقم الهاتف يجب أن يحتوي على 10 أرقام'),
    government: Yup.string().required('المحافظة مطلوبة'),
    password: Yup.string()
      .required('كلمة المرور مطلوبة')
      .matches(/^\d{6}$/, 'كلمة المرور يجب أن تحتوي على 6 أرقام'),
    email: Yup.string()
        ...
        <form className={style.forms} onSubmit={handleSubmit}>
          ...
          <div className={style.inform}>
            <div className={style.input}>
              <div className={style.inpi}>
                ...
                {errors.username && touched.username && (
                  <p className={`alert alert-danger p-1 fw-1 ${style.alert}`}>
                    {errors.username}
                  </p>
                )}
              </div>
            ...
            <div className={style.input}>
              ...
              <div className={style.inpi}>
                ...
                <input
                  ...
                  onBlur={handleBlur}
                />
                {errors.government && touched.government && (
                  <p className={`alert alert-danger p-1 fw-1 ${style.alert}`}>
                    {errors.government}
                  </p>
                )}
            ...
            <div className={style.btnInpu}>
              ...
              <button onClick={() => setOpenAuth('login')}>
                لدي حساب بالفعل
              </button>
              {errorMsg && (
                <div
                  className={`form-control  alert alert-danger m-3 p-1 w-75 text-center ${style.alert}`}
                >
                  {errorMsg}
                </div>
              )}
